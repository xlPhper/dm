 ...
 * @since 2014/10/24
 */
class DM_Daemon_MemoryCheck extends DM_Daemon_Base
{
    ...
    public function check()
    {
        ...
        if ($system*$this->percentLimit>=$this->getMemoryKeep()){//如果 system*percent>=keep，则用keep判断。
            if ($system-memory_get_usage()<$this->getMemoryKeep()){
                $message="Detect Out of memory service ".$this->service.": System ".ini_get('memory_limit')." used:".self::formatMemory(memory_get_usage())." spare:".self::formatMemory($system-memory_get_usage())."<keep:".$this->spareMemoryKeep."M Trigger event.";
                $log->add($message);
                $log->flush();
                return false;
            }
            ...
            if (memory_get_peak_usage()>$system*$this->percentLimit){
                $message="Detect Out of memory service ".$this->service.": System ".self::getMemoryLimit()." used:".self::formatMemory(memory_get_usage())." pecent:".(number_format(memory_get_peak_usage()/ini_get('memory_limit'), 2, '.', '')*100)."% >limit ".($this->percentLimit*100)."% Trigger event.";
                $log->add($message);
                $log->flush();
                return false;
            }
